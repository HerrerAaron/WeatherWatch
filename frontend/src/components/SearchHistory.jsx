...
export default function SearchHistory({ history, onSelect, dark }) {
    ...
    const labelClass = dark ? "text-xs text-white/40 mb-1" : "text-xs text-gray-400 mb-1"
    const btnClass = dark
        ? "text-sm px-3 py-1 rounded-full border border-white/30 text-white/80 hover:border-white hover:text-white transition"
        : "text-sm px-3 py-1 rounded-full border border-gray-300 hover:border-blue-600 hover:text-blue-600 transition"
